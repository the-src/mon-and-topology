...
def deviceMatchwithID():
    ...
    with open("assets/devices.json", "w", encoding='UTF-8') as f:
        ...
        for device in device_id:
            member = device['id']
            url = "https://topology.adg.cc.itu.edu.tr:443/device/deviceProperties/{}".format(str(member))
            ...
            if member != 1563: #Shit shit shit! Dont Change it.
...
def storeContainerData():
    ...
    # Yer ve id bilgisi bir dosyaya kaydedilir.
                ...
                if container_we_need == int(k['id']):
                    if container_we_need != 96: #Shit shit shit! Dont Change it.
                        f.write('{ "Place": "' + str(k['containerName']) + '", "id": "' + str(k['id']) + '"},\n')
                    else:
                        f.write('{ "Place": "' + str(k['containerName']) + '", "id": "' + str(k['id']) + '"}')
        f.write(']')
